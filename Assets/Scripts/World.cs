...
public class World : MonoBehaviour
{
    ...
    public byte GetVoxel (Vector3 pos) {
        ...
        float radius = (VoxelData.WorldSizeInVoxels / 3);
        float middlePoint = VoxelData.WorldSizeInVoxels / 2;
        float distance = Mathf.Sqrt(Mathf.Pow(pos.x - middlePoint, 2) + Mathf.Pow(pos.z- middlePoint , 2));
        
        int edgeForLand = (VoxelData.WorldSizeInVoxels / 5) + 3;

        bool outInsel = distance > radius;
        bool beach = distance <= radius && distance > (radius - 3);
        
//        bool waterSide = xPos + edgeForWater > VoxelData.WorldSizeInVoxels || xPos - edgeForWater < 0 ||
//                         zPos + edgeForWater > VoxelData.WorldSizeInVoxels || zPos - edgeForWater < 0;
//        
//        bool bevorWaterSide = xPos + edgeForLand > VoxelData.WorldSizeInVoxels || xPos - edgeForLand < 0 ||
//                         zPos + edgeForLand > VoxelData.WorldSizeInVoxels || zPos - edgeForLand < 0;
//        
        ...
        if (outInsel)
        {
            terrainHeight = solidGroundHeight + (solidGroundHeight/5);
        }
